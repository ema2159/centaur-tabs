...
** Mouse support
   ...
   - On the left of the header bar, there are navigation buttons for backward/forward the current tab, and show the tab groups menu. To get back the legacy look&feel, you can disable the buttons with the =centaur-tabs-show-navigation-buttons= customized variable.
